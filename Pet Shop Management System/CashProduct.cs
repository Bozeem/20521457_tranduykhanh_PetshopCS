    ...
    public partial class CashProduct : Form
    {
        ...
        private void btnCash_Click(object sender, EventArgs e)
        {
            foreach(DataGridViewRow dr in dgvProduct.Rows)
            {
                ...
                if(chkbox)
                {
                    if (cash.checkPcode(dr.Cells[1].Value.ToString()) == false)
                    {
                        try
                        {
                            ...
                            cm.Parameters.AddWithValue("@price", Convert.ToDouble(dr.Cells[6].Value.ToString()));
                            cm.Parameters.AddWithValue("@cashier", uname);
                            
                            string pcode = dr.Cells[1].Value.ToString();
                            cn.Open();
                            cm.ExecuteNonQuery();
                            cn.Close();
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT pcode, pname, ptype, pcategory,c.name, pprice FROM tbProduct pro LEFT JOIN tbCategory AS c ON pro.pcategory = c.cateid WHERE  CONCAT(pname,ptype,pcategory) LIKE '%" + txtSearch.Text + "%' AND pqty > "+0+"", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void CashProduct_FormClosing(object sender, FormClosingEventArgs e)
        {
